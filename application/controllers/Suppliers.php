...
class Suppliers extends MY_Controller {
	...
	public function ajax_list()
	{
		...
		foreach ($list as $suppliers) {
			...
			$row[] = $suppliers->email;
			$q6 = $this->db->query("select coalesce(sum(grand_total),0) as total_purchase_amount,coalesce(sum(paid_amount),0) as total_paid_amount from db_purchase where supplier_id=$suppliers->id");
			$total_purchase_amount = $q6->row()->total_purchase_amount;
			$row[] = (!empty($total_purchase_amount)) ? app_number_format($total_purchase_amount) : (0);
