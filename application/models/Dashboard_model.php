...
class Dashboard_model extends CI_Model
{
	...
	public function breadboard_values()
	{
		...
		//Month SAles amount
		$query66="SELECT COALESCE(sum(grand_total),0) AS tot_sal_month_total FROM db_sales where MONTH(sales_date) = MONTH(NOW())";
		$tot_sal_month_total=$this->db->query($query66)->row()->tot_sal_month_total;

		//Total Purchase amount
		$query666="SELECT COALESCE(sum(grand_total),0) AS tot_pur_month_total FROM db_purchase WHERE MONTH(purchase_date) = MONTH(NOW())";
		$tot_pur_month_total=$this->db->query($query666)->row()->tot_pur_month_total;

		//Month Purchase amount
		$query6661="SELECT COALESCE(sum(grand_total),0) AS tot_pur_grand_total FROM db_purchase";
		$tot_pur_grand_total=$this->db->query($query6661)->row()->tot_pur_grand_total;
		...
		//Month SAles return amount
		$query611="SELECT COALESCE(sum(grand_total),0) AS tot_month_sales_return FROM db_salesreturn WHERE MONTH(return_date) = MONTH(NOW())";
		$tot_month_sales_return=$this->db->query($query611)->row()->tot_month_sales_return;

		//Total Purchase return amount
		$query616="SELECT COALESCE(sum(grand_total),0) AS tot_purchase_return FROM db_purchasereturn";
		$tot_purchase_return=$this->db->query($query616)->row()->tot_purchase_return;

		//Month Purchase return amount
		$query6116="SELECT COALESCE(sum(grand_total),0) AS tot_month_purchase_return FROM db_purchasereturn WHERE MONTH(return_date) = MONTH(NOW())";
		$tot_month_purchase_return=$this->db->query($query6116)->row()->tot_month_purchase_return;
		...
		$data['tot_sal_month_total']=($tot_sal_month_total-$tot_month_sales_return);
		$data['tot_pur_grand_total']=($tot_pur_grand_total-$tot_purchase_return);
		$data['tot_pur_month_total']=($tot_pur_month_total-$tot_month_purchase_return);
