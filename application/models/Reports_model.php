...
class Reports_model extends CI_Model {
	...
	public function show_sales_report(){
		...
		// } else {
			$view_all = "no";
		// }
		$q2=$this->db->query("select coalesce(SUM(db_salesitems.sales_qty*db_salesitems.price_per_unit),0) as penjualan from db_salesitems,db_sales where db_salesitems.sales_id = db_sales.id AND db_sales.sales_date>='$from_date' and db_sales.sales_date<='$to_date'");
		$tot_penjualan=$q2->row()->penjualan;

		$q4=$this->db->query("select coalesce(SUM(db_salesitemsreturn.return_qty*db_salesitemsreturn.price_per_unit),0) as retur_penjualan from db_salesitemsreturn,db_salesreturn where db_salesitemsreturn.return_id = db_salesreturn.id and db_salesreturn.return_date>='$from_date' and db_salesreturn.return_date<='$to_date'");
		$tot_retur_penjualan=$q4->row()->retur_penjualan;

		$q3=$this->db->query("select coalesce(SUM(db_salesitems.discount_amt),0) as potongan_penjualan from db_salesitems,db_sales where db_salesitems.sales_id = db_sales.id AND db_sales.sales_date>='$from_date' and db_sales.sales_date<='$to_date'");
		$tot_potongan_penjualan=$q3->row()->potongan_penjualan;
		
		$this->db->select("a.id,a.sales_code,a.sales_date,b.customer_name,b.customer_code,a.subtotal,a.other_charges_amt as angkut ,a.grand_total,a.paid_amount, c.grand_total as return_grand_total, c.subtotal as retur");
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				// echo "<td class='text-right'>".app_number_format($res1->return_grand_total)."</td>";
				// echo "<td class='text-right'>".app_number_format($res1->grand_total-$res1->return_grand_total)."</td>";
				// // echo "<td class='text-left'>$date_difference</td>";
				// echo "</tr>";
				$tot_retur+=$res1->retur;
				$tot_subtotal+=$res1->subtotal;
				$tot_grand_total+=$res1->grand_total;
				$tot_return_grand_total+=$res1->return_grand_total;
				$tot_due_amount+=($res1->grand_total-$res1->paid_amount);

			}
			echo "<tr>
					<td class='text-center text-bold' colspan='3'>LAPORAN PENJUALAN <br> Periode ".$from_date." sampai dengan ".$to_date."</td>
				  </tr>
				  <tr>
				  	<td style='width: 30%; text-indent:100px;'>Penjualan</td>
				  	<td></td>
				  	<td>Rp. ".app_number_format($tot_penjualan)."</td>
				  </tr>
				  <tr>
				  	<td style='text-indent:100px;'>Retur Penjualan</td>
				  	<td>Rp. ".app_number_format($tot_retur_penjualan)."</td>
				  	<td></td>
				  </tr>
				  <tr>
				  	<td style='text-indent:100px;'>Potongan Penjualan</td>
				  	<td style='border-bottom: thin solid;'>Rp. ".app_number_format($tot_potongan_penjualan)."</td>
				  	<td></td>
				  </tr>
				  <tr>
				  	<td></td>
				  	<td></td>
				  	<td style='border-bottom: thin solid;'>Rp. ".app_number_format($tot_retur_penjualan+$tot_potongan_penjualan)."</td>
				  </tr>
				  <tr>
				  	<td style='text-indent:100px;'><strong>Penjualan Bersih</strong></td>
				  	<td></td>
				  	<td><strong>Rp. ".app_number_format($tot_penjualan-($tot_retur_penjualan+$tot_potongan_penjualan))."</strong><br><br></td>
				  </tr>
				  ";
			...
			// 		  <td class='text-right text-bold'>".app_number_format($tot_return_grand_total)."</td>
			// 		  <td class='text-right text-bold'>".app_number_format($tot_grand_total - $tot_return_grand_total)."</td>
			// 		  <td></td>
			// 	  </tr>";
		}
		else{
	...
	public function show_purchase_report(){
		...
		// } else {
			$view_all = "no";
		// }
		$q2=$this->db->query("select coalesce(SUM(db_purchaseitems.purchase_qty*db_purchaseitems.price_per_unit),0) as pembelian from db_purchaseitems,db_purchase where db_purchase.id = db_purchaseitems.purchase_id AND db_purchase.purchase_date>='$from_date' and db_purchase.purchase_date<='$to_date'");
		$tot_pembelian=$q2->row()->pembelian;

		$q3=$this->db->query("select coalesce(SUM(db_purchaseitemsreturn.return_qty*db_purchaseitemsreturn.price_per_unit),0) as retur_pembelian from db_purchaseitemsreturn,db_purchasereturn where db_purchasereturn.id = db_purchaseitemsreturn.return_id AND db_purchasereturn.return_date>='$from_date' and db_purchasereturn.return_date<='$to_date'");
		$tot_retur_pembelian=$q3->row()->retur_pembelian;

		$q3=$this->db->query("select coalesce(SUM(db_purchaseitems.discount_amt+(db_purchase.tot_discount_to_all_amt/10)),0) as potongan_pembelian from db_purchaseitems,db_purchase where db_purchase.id = db_purchaseitems.purchase_id AND db_purchase.purchase_date>='$from_date' and db_purchase.purchase_date<='$to_date'");
		$tot_potongan_pembelian=$q3->row()->potongan_pembelian;

		// $this->db->select("a.id,a.purchase_code,a.purchase_date,a.other_charges_input, b.supplier_name,b.supplier_code,a.grand_total,a.paid_amount, c.grand_total as retur_grand_total");
		$this->db->select("a.id,a.purchase_code,a.purchase_date,a.other_charges_input,a.grand_total,a.paid_amount");
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				// echo "<td class='text-right'>".app_number_format($res1->grand_total)."</td>";
				// echo "<td class='text-right'>".app_number_format($res1->retur_grand_total)."</td>";
				// echo "<td class='text-right'>".app_number_format($res1->grand_total - $res1->retur_grand_total)."</td>";
				// // echo "<td class='text-left'>$date_difference</td>";
				// echo "</tr>";
				$tot_grand_total+=$res1->grand_total;
				$tot_other_charges_input+=$res1->other_charges_input;
				$tot_retur_grand_total+=$res1->retur_grand_total;
				// $tot_due_amount+=($res1->grand_total-$res1->paid_amount);

			}

			echo "	<tr>
						<td class='text-center text-bold' colspan='3'>LAPORAN PEMBELIAN<br> Periode ".$from_date." sampai dengan ".$to_date."</td>
					</tr>
					<tr>
						<td style='width: 30%;text-indent:100px;'>Pembelian</td>
						<td></td>
						<td>Rp. ".app_number_format($tot_pembelian)."</td>
					</tr>
					<tr>
						<td style='text-indent:100px;'>Beban Angkut Pembelian</td>
						<td></td>
						<td style='border-bottom: thin solid;'>Rp. ".app_number_format($tot_other_charges_input)."</td>
					</tr>
					<tr>
						<td></td>
						<td></td>
						<td> Rp. ".app_number_format($tot_pembelian+$tot_other_charges_input)."</td>
					</tr>
					<tr>
						<td style='text-indent:100px;'>Retur Pembelian</td>
						<td> Rp. ".app_number_format($tot_retur_pembelian)."</td>
						<td></td>
					</tr>
					<tr>
						<td style='text-indent:100px;'>Potongan Pembelian</td>
						<td style='border-bottom: thin solid;'>Rp. ".app_number_format($tot_potongan_pembelian)."</td>
						<td></td>
					</tr>
					<tr>
						<td></td>
						<td></td>
						<td style='border-bottom: thin solid;'>Rp. ".app_number_format($tot_retur_pembelian+$tot_potongan_pembelian)."</td>
					</tr>
					<tr>
						<td style='text-indent:100px;'><strong>Pembelian Bersih</strong></td>
						<td></td>
						<td><strong>Rp. ".app_number_format(($tot_pembelian+$tot_other_charges_input)-($tot_retur_pembelian+$tot_potongan_pembelian))."</strong><br><br></td>
					</tr>
				  ";
			...
			// 		  <td class='text-right text-bold' colspan='5'><b>Total :</b></td>
			// 		  <td class='text-right text-bold'>".app_number_format($tot_grand_total)."</td>
			// 		  <td class='text-right text-bold'>".app_number_format($tot_retur_grand_total)."</td>
			// 		  <td class='text-right text-bold'>".app_number_format($tot_grand_total - $tot_retur_grand_total)."</td>
	...
	public function stok_awal($item_id, $from_date){

		$q7=$this->db->query("select COALESCE(SUM(qty),0) as stock_qty from db_stockentry where item_id='$item_id' and entry_date<='$from_date'");
		$stock_qty=$q7->row()->stock_qty;
		
		$q8=$this->db->query("select COALESCE(SUM(purchase_qty),0) as pu_tot_qty from db_purchaseitems where item_id='$item_id' and purchase_status='Received' and created_at<='$from_date'");
		$pu_tot_qty=$q8->row()->pu_tot_qty;
				
		$q9=$this->db->query("select coalesce(SUM(sales_qty),0) as sl_tot_qty from db_salesitems where item_id='$item_id' and sales_status='Final' and created_at<='$from_date'");
		$sl_tot_qty=$q9->row()->sl_tot_qty;
		
		/*Fid Return Items Count*/
		$q6=$this->db->query("select COALESCE(SUM(return_qty),0) as pu_return_tot_qty from db_purchaseitemsreturn where item_id='$item_id' and created_at<='$from_date'");/*and purchase_id is null */
		$pu_return_tot_qty=$q6->row()->pu_return_tot_qty;
		
		/*Fid Return Items Count*/
		$q6=$this->db->query("select COALESCE(SUM(return_qty),0) as sl_return_tot_qty from db_salesitemsreturn where item_id='$item_id' and created_at<='$from_date'");/*and sales_id is null */
	...
	public function stok_akhir($item_id, $from_date, $to_date){

		$q7=$this->db->query("select COALESCE(SUM(qty),0) as stock_qty from db_stockentry where item_id='$item_id' and entry_date>='$from_date' and entry_date<='$to_date'");
		$stock_qty=$q7->row()->stock_qty;
		
		$q8=$this->db->query("select COALESCE(SUM(purchase_qty),0) as pu_tot_qty from db_purchaseitems where item_id='$item_id' and purchase_status='Received' and created_at>='$from_date' and created_at<='$to_date'");
		$pu_tot_qty=$q8->row()->pu_tot_qty;
				
		$q9=$this->db->query("select coalesce(SUM(sales_qty),0) as sl_tot_qty from db_salesitems where item_id='$item_id' and sales_status='Final' and created_at>='$from_date' and created_at<='$to_date'");
		$sl_tot_qty=$q9->row()->sl_tot_qty;
		
		/*Fid Return Items Count*/
		$q6=$this->db->query("select COALESCE(SUM(return_qty),0) as pu_return_tot_qty from db_purchaseitemsreturn where item_id='$item_id' and created_at>='$from_date' and created_at<='$to_date'");/*and purchase_id is null */
		$pu_return_tot_qty=$q6->row()->pu_return_tot_qty;
		
		/*Fid Return Items Count*/
		$q6=$this->db->query("select COALESCE(SUM(return_qty),0) as sl_return_tot_qty from db_salesitemsreturn where item_id='$item_id' and created_at>='$from_date' and created_at<='$to_date'");/*and sales_id is null */
		$sl_return_tot_qty=$q6->row()->sl_return_tot_qty;
	...
	public function show_stock_report2(){
		extract($_POST);
		...
		$this->db->join("db_brands as c","c.id=a.brand_id","left");
		$this->db->join("db_category as d","d.id=a.category_id","left");
		$this->db->join('db_units as e','e.id=a.unit_id','left');
		...
		if($q1->num_rows()>0){
			...
			foreach ($q1->result() as $res1) {
				...
				// } else {
					$stock_qty = $this->stok_awal($item_id, $from_date);
					$stock = $this->stok_akhir($item_id, $from_date, $to_date);
				// }
				
				$tax_type = ($res1->tax_type=='Inclusive') ? 'Inc.' : 'Exc.';
				$stock_value = $res1->price * $stock;
				$str.="<tr>";
				$str.="<td>".++$i."</td>";
				$str.="<td>".$res1->item_code."</td>";
				$str.="<td>".$res1->item_name."</td>";
				// $str.="<td>".$res1->brand_name."</td>";
				// $str.="<td>".$res1->category_name."</td>";
				$str.="<td>".$res1->unit_name."</td>";
				// $str.="<td class='text-right'>".app_number_format($res1->purchase_price)."</td>";
				// $str.="<td>".$res1->tax_name."[".$tax_type."]</td>";
				$str.="<td class='text-right'>".app_number_format($res1->price)."</td>";
				// $str.="<td>".$stock_qty."</td>";
				$str.="<td>".$stock."</td>";
				$str.="<td class='text-right'>".app_number_format($stock_value)."</td>";
				$str.="</tr>";
				$tot_start_stock+=$stock_qty;
				$tot_sales_price+=$res1->sales_price;
				$tot_stock_value+=$stock_value;
				$tot_stock+=$stock;
