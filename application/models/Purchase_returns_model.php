...
class Purchase_returns_model extends CI_Model {
	...
	public function purchase_list($purchase_id){
		...
		foreach ($q1->result() as $res1) {
			$q2=$this->db->query("select item_name,item_code,stock,tax_type,price from db_items where id=".$res1->item_id);
			...
			$info['item_purchase_price'] = $q2->row()->price;
	...
	public function return_row_with_data($rowcount,$info){
		...
		$item_amount = $item_purchase_price * $item_purchase_qty;
		...
		?>
            <tr id="row_<?=$rowcount;?>" data-row='<?=$rowcount;?>'>
               ...
               <!-- Purchase Price -->
               <td id="td_<?=$rowcount;?>_4"><input type="text" name="td_data_<?=$rowcount;?>_4" id="td_data_<?=$rowcount;?>_4" class="form-control text-right no-padding only_currency text-center"  onkeyup="calculate_tax(<?=$rowcount;?>)" value="<?=$item_price;?>" ></td>
